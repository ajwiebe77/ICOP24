### holzbecher_v10.R
### Andrew J. Wiebe, 17 May 2023 (2 Aug 2023 - decrease number of streamlines for Whati case)
#
# Objective: Remove branch cuts from contour lines by using the sequential contouring method by Holzbecher (2018)
#
# Code slightly modified from the script holzbecher_v10.R at https://github.com/ajwiebe77/theYWVC (Wiebe and McKenzie, 2022)
#
...
holzbecher_v10nopf <- function(psi, zD_xy, wells, consts, xy_stg){
  ...
  numContoursStr <- 5 # 15 # 15 - 2 Aug 2023; 10 - 1 Jan 2024
  ...
  if(nrow(wells2) > 1){ # add this 3 Jan 2023
    ...
    nspace <- ceiling(abs(wells2[which(wells2[,2] == max(wells2[,2])),3]) / spacing) * 2 # account for an injection well being at max y location  # 1 Jan 2024
    ...
    nspace <- ceiling(abs(wells2[1,3]) / spacing) * 2 # account for an injection well being at max y location  # 1 Jan 2024
    nspace1 <- nspace
    spacing <- wells2[1,3] / nspace
  }
  
  spacing <- abs(spacing) # account for an injection well being at max y location # 1 Jan 2024
  
  print(c(minz,maxz,spacing))
  ...
  if(nrow(wells2) > 1){
    ...
    for(i in seq(N,1,by=-1)){
      ...
      if(i < N){
        for(x in 1:n){
          for(y in startIndx:stopIndx){
            if(y > 0){ # 12 Dec 2023
              if(is.finite(psi[x,y])){ # helpful for case 2 option 3 # 14 Dec 2023
                if(psi[x,y] > 0){
      ...
      if(nrow(wells2) > 1){ # add this 3 Jan 2023
        ...
        nspace <- nspace1 * abs(wells2[i,3]/wells2[which(wells2[,2] == max(wells2[,2])),3]) # try absolute value; something goes wrong when the negative (injection well) is processed - 24 Apr 2023 # revised 1 Jan 2024
      ...
      spacing <- abs(wells2[i,3] / nspace) # try absolute value; something goes wrong when the negative (injection well) is processed - 24 Apr 2023 # revised 1 Jan 2024
  ...
  # plot(c(0,r_wedge), c(0,0), xlim=c(0,r_wedge), ylim=c(0,r_wedge))
