### trimAtBoundariesIMPERM3.R
### Andrew J. Wiebe, 1 Jan 2024
#
# Remove points outside aquifer for third side of wedge-shaped/triangular-shaped aquifer that is semi-infinite.
#
...
trimAtBoundariesIMPERM3 <- function(list1, case, option, alpha, axisLen, xy, mirrorIMPERMEABLE){
  list2 <- list1 
      ...
      if(xy == TRUE){
        for(ii in 1:length(list1[[i]]$x)){ # for each point
          if( ! is.na(list1[[i]]$x[ii])){
            if(list1[[i]]$x[ii] > 0){
              if(list1[[i]]$x[ii] > axisLen || list1[[i]]$y[ii] < 0 || (mirrorIMPERMEABLE == FALSE && list1[[i]]$y[ii] > list1[[i]]$x[ii] * tan(alpha)) || (mirrorIMPERMEABLE == FALSE && sqrt((list1[[i]]$x[ii])^2 + (list1[[i]]$y[ii])^2) > axisLen)){ # revised 1 Jan 2024
                list2[[i]]$x[ii] <- NaN
                list2[[i]]$y[ii] <- NaN
              }
