...
%            Plot a contour map showing the hydraulic head contours and the splines.
% 
% Notes:
...
figure;
hold on;
plot(xc*cos(alpha/2),xc*sin(alpha/2),'k-');
...
contour(xi2,yi2,zzz2);

plot([r*cos(alpha), 0, r], [r*sin(alpha), 0, 0], 'b-', 'linewidth', 2);
colormap("viridis");
colorbar;

gca1 = gca();
set(gca1, 'fontsize', 14)

xlabel('X (m)','fontsize',14);
ylabel('Y (m)','fontsize',14);
% text(1060,100,'Hydraulic head, relative to lake level (m)','rotation', 90, 'fontsize', 14);
text(530,50,'Hydraulic head, relative to lake level (m)','rotation', 90, 'fontsize', 14);
axis([0 500 0 500]);
hold off;

print -dpng -r300 splines3.png;
